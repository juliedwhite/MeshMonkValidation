...
landmarks <- read_excel("C:/Users/Julie White/Desktop/RemappingValidation/PeterLandmarks/SparseLandmarkingManualAndAutomaticv20180226LOO.xlsx", sheet = "Combined")
...
```{r}
#Make Arslan average of Intra values
ArslanIntra$A_ml_avg_euclid_tocent <- apply(ArslanIntra[10:12],1, function(x) mean(x))
ArslanIntra$A_ml_avg_euclid_tocent <- as.numeric(as.character(ArslanIntra$A_ml_avg_euclid_tocent))

#Make Julie average of Intra values
JulieIntra$J_ml_avg_euclid_tocent <- apply(JulieIntra[10:12],1, function(x) mean(x))
JulieIntra$J_ml_avg_euclid_tocent <- as.numeric(as.character(JulieIntra$J_ml_avg_euclid_tocent))

#Make new dataframe with comparison
ML_ToAvg_CompareInterIntra <- as.data.frame(cbind(ArslanIntra$ID, ArslanIntra$LM.Num, ArslanIntra$LM.Name, ArslanIntra$A_ml_avg_euclid_tocent, JulieIntra$J_ml_avg_euclid_tocent))
colnames(ML_ToAvg_CompareInterIntra) <- c("ID", "LM.Num", "LM.Name", "A_ml_avg_euclid_tocent", "J_ml_avg_euclid_tocent")
...
#WHAT IS THE APPROPRIATE INTER ERROR HERE# 
```
