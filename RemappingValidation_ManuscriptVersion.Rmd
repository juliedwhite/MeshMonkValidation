...
```{r}
BothManualGroups <- as.data.frame(matrix(nrow=nrow(Both_MorphoData_Manual), ncol=6))
colnames(BothManualGroups) <- c("ID", "StudyID", "Camera", "Observer", "Method", "Iteration")
BothManualGroups$ID <- rownames(Both_MorphoData_Manual)
for (i in 1:nrow(Both_MorphoData_Manual)){
  BothManualGroups[i,2] <- strsplit(rownames(Both_MorphoData_Manual), split = "_")[[i]][1]
  BothManualGroups[i,6] <- strsplit(rownames(Both_MorphoData_Manual), split = "_")[[i]][4]
}
BothManualGroups$Camera <- c(rep("GHPAFF 2pod", each=14), "UIUC 2pod", rep("ADAPT 3pod", each = 26))
BothManualGroups$Observer <- rep("Both")
BothManualGroups$Method <- rep("ML")
```
...
```
Make groups dataframe that has sample information for all of AZ auto data 
```{r}
AZAutoGroups <- as.data.frame(matrix(nrow=nrow(AZ_MorphoData_Auto), ncol=6))
colnames(AZAutoGroups) <- c("ID", "StudyID", "Camera", "Observer", "Method", "Iteration")
AZAutoGroups$ID <- rownames(AZ_MorphoData_Auto)
for (i in 1:nrow(AZ_MorphoData_Auto)){
  AZAutoGroups[i,2] <- strsplit(rownames(AZ_MorphoData_Auto), split = "_")[[i]][1]
  AZAutoGroups[i,6] <- strsplit(rownames(AZ_MorphoData_Auto), split = "_")[[i]][4]
}
AZAutoGroups$Camera <- c(rep("GHPAFF 2pod", each=14), "UIUC 2pod", rep("ADAPT 3pod", each = 26))
AZAutoGroups$Observer <- rep("AZ")
AZAutoGroups$Method <- rep("Auto")
```
###JW Auto
Make empty dataframe to store landmarks
```{r}
...
```
Make groups dataframe that has sample information for all of JW auto data 
```{r}
JWAutoGroups <- as.data.frame(matrix(nrow=nrow(JW_MorphoData_Auto), ncol=6))
colnames(JWAutoGroups) <- c("ID", "StudyID", "Camera", "Observer", "Method", "Iteration")
JWAutoGroups$ID <- rownames(JW_MorphoData_Auto)
for (i in 1:nrow(JW_MorphoData_Auto)){
  JWAutoGroups[i,2] <- strsplit(rownames(JW_MorphoData_Auto), split = "_")[[i]][1]
  JWAutoGroups[i,6] <- strsplit(rownames(JW_MorphoData_Auto), split = "_")[[i]][4]
}
JWAutoGroups$Camera <- c(rep("GHPAFF 2pod", each=14), "UIUC 2pod", rep("ADAPT 3pod", each = 26))
JWAutoGroups$Observer <- rep("JW")
JWAutoGroups$Method <- rep("Auto")
```
###Both auto
Make empty dataframe to store landmarks
```{r}
...
```
Make groups dataframe that has sample information for all of Both auto data 
```{r}
BothAutoGroups <- as.data.frame(matrix(nrow=nrow(Both_MorphoData_Auto), ncol=6))
colnames(BothAutoGroups) <- c("ID", "StudyID", "Camera", "Observer", "Method", "Iteration")
BothAutoGroups$ID <- rownames(Both_MorphoData_Auto)
for (i in 1:nrow(Both_MorphoData_Auto)){
  BothAutoGroups[i,2] <- strsplit(rownames(Both_MorphoData_Auto), split = "_")[[i]][1]
  BothAutoGroups[i,6] <- strsplit(rownames(Both_MorphoData_Auto), split = "_")[[i]][4]
}
BothAutoGroups$Camera <- c(rep("GHPAFF 2pod", each=14), "UIUC 2pod", rep("ADAPT 3pod", each = 26))
BothAutoGroups$Observer <- rep("Both")
BothAutoGroups$Method <- rep("Auto")
```
...
AJ_Manual_Groups$StudyID <- c(rep(unique(landmarks$ID)))
AJ_Manual_Groups$Iteration <- rep(c(rep("1", each=41), rep("2", each=41), rep("3", each=41)))
AJ_Manual_Groups$Camera <- rep(c(rep("GHPAFF 2pod", each=14), "UIUC 2pod", rep("ADAPT 3pod", each = 26)))
AJ_Manual_Groups$Observer <- c(rep("AZ", each=123), rep("JW", each=123))
...
AJ_Morpho_Manual.manova <- procD.lm(shape ~ individual + observer + iteration + individual*observer + camera + individual*camera, data = AJ_Manual_GDF, iter = 999, RRPP=TRUE, print.progress=FALSE)
...
##Manova of manual vs automatic landmarks, using just "Both" data
...
Both_MLvsAuto_Groups$StudyID <- c(rep(unique(landmarks$ID)))
Both_MLvsAuto_Groups$Camera <- rep(c(rep("GHPAFF 2pod", each=14), "UIUC 3 pod", rep("ADAPT 3pod", each = 26)))
Both_MLvsAuto_Groups$Method <- c(rep("ML", each=41), rep("Auto", each=41))
```
...
```
###Perform manova with study individual, camera, and method as covariates
```{r}
#Make geomorph dataframe 
...
Both_Morpho_MLvsAuto.manova <- procD.lm(shape ~ individual + method + camera + individual*camera, data = Both_MLvsAuto_GDF, iter = 999, RRPP=TRUE, print.progress=FALSE)
...
CentroidSizes$Camera <- c(rep("GHPAFF 2pod", each=14), "UIUC 2pod", rep("ADAPT 3pod", each = 26))
...
ba_centroid_Adf <- as.data.frame(matrix(NA, nrow = 779))
...
ba_centroid_Jdf <- as.data.frame(matrix(NA, nrow = 779))
...
ba_centroid_Bdf <- as.data.frame(matrix(NA, nrow = 779))
...
```{r}
ggplot(data=melt(CentroidSizes, id.vars = "ID"), aes(x=variable, y=value))+geom_boxplot(outlier.shape = NA)+geom_jitter(width = 0.2, size = 0.8, alpha = 0.5)+labs(title="Comparison of centroid sizes", x="Measurement method", y="Centroid size (mm)")+theme_bw()+scale_x_discrete(breaks = c("A_Manual", "A_Auto", "J_Manual", "J_Auto", "B_Manual", "B_Auto"), labels = c("AZ manual", "AZ auto", "JW manual", "JW auto", "Both manual", "Both auto"))
```
