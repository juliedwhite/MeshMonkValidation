...
MLvsAuto_RMSE_ByLandmark <- ddply(MLvsAuto,~LM.Name,summarise, 
                                  ...
                                  Jml_Jloo_rmsex = mean(Jml_Jloo_rmsex), 
                                  Jml_Jloo_rmsey = mean(Jml_Jloo_rmsey), 
                                  Jml_Jloo_rmsez = mean(Jml_Jloo_rmsez), 
                                  ...
                                  Jml_Bloo_rmsex = mean(Jml_Bloo_rmsex),
                                  Jml_Bloo_rmsey = mean(Jml_Bloo_rmsey),
                                  Jml_Bloo_rmsez = mean(Jml_Bloo_rmsez),
                                  ...
                                  Bml_Jloo_rmsex = mean(Bml_Jloo_rmsex), 
                                  Bml_Jloo_rmsey = mean(Bml_Jloo_rmsey), 
                                  Bml_Jloo_rmsez = mean(Bml_Jloo_rmsez), 
...
```{r}
AJ_Manual_Groups <- as.data.frame(matrix(nrow=nrow(AJ_MorphoData_Manual), ncol=4))
colnames(AJ_Manual_Groups) <- c("ID", "StudyID", "Observer", "Iteration")
AJ_Manual_Groups$ID <- rownames(AJ_MorphoData_Manual)
AJ_Manual_Groups$StudyID <- as.factor(unique(landmarks$ID))
AJ_Manual_Groups$Iteration <- as.factor(c(rep("1", each=41), rep("2", each=41), rep("3", each=41)))
AJ_Manual_Groups$Observer <- as.factor(c(rep("AZ", each=123), rep("JW", each=123)))
```
GPA align
```{r}
...
plotAllSpecimens(AJ_Morpho_Manual.gpa$coords)
```
No outliers
Make geomorph dataframe 
```{r}
AJ_Manual_GDF <- geomorph.data.frame(shape = AJ_Morpho_Manual.gpa$coords, individual = AJ_Manual_Groups$StudyID, observer = AJ_Manual_Groups$Observer, iteration = AJ_Manual_Groups$Iteration)

AJ_Morpho_Manual.manova <- procD.lm(f1 = shape ~ individual+observer*individual+observer/iteration, data = AJ_Manual_GDF, RRPP=TRUE, iter = 999, print.progress=FALSE)
...
##Manoca of manual average landmarks with individual and observer as covariates
```{r}
AJ_MorphoData_ManualAvg <- rbind(AZ_MorphoData_Manual_avg, JW_MorphoData_Manual_avg)
```
```{r}
AJ_ManualAvg_Groups <- as.data.frame(matrix(nrow = nrow(AJ_MorphoData_ManualAvg), ncol = 3))
colnames(AJ_ManualAvg_Groups) <- c("ID", "StudyID", "Observer")
AJ_ManualAvg_Groups$ID <- rownames(AJ_MorphoData_ManualAvg)
AJ_ManualAvg_Groups$StudyID <- as.factor(unique(landmarks$ID))
AJ_ManualAvg_Groups$Observer <- as.factor(c(rep("AZ", each=41), rep("JW", each=41)))
```
```{r}
AJ_Morpho_ManualAvg <- arrayspecs(A=AJ_MorphoData_ManualAvg, p=19, k=3)
AJ_Morpho_ManualAvg.gpa <- gpagen(A=AJ_Morpho_ManualAvg, print.progress = FALSE)
```
```{r}
plotOutliers(AJ_Morpho_ManualAvg.gpa$coords)
plotAllSpecimens(AJ_Morpho_ManualAvg.gpa$coords)
```
```{r}
AJ_ManualAvg_GDF <- geomorph.data.frame(shape = AJ_Morpho_ManualAvg.gpa$coords, individual = AJ_ManualAvg_Groups$StudyID, observer = AJ_ManualAvg_Groups$Observer)

AJ_Morpho_ManualAvg.manova <- procD.lm(shape~observer+individual, data = AJ_ManualAvg_GDF, print.progress = FALSE)
summary(AJ_Morpho_ManualAvg.manova)
```
```{r}
plot(AJ_Morpho_ManualAvg.manova, type = "diagnostics", outliers = TRUE)
```
...
```{r}
AJ_Auto_Groups <- as.data.frame(matrix(nrow=nrow(AJ_MorphoData_Auto), ncol=3))
colnames(AJ_Auto_Groups) <- c("ID", "StudyID", "Observer")
AJ_Auto_Groups$ID <- rownames(AJ_MorphoData_Auto)
AJ_Auto_Groups$StudyID <- as.factor(unique(landmarks$ID))
AJ_Auto_Groups$Observer <- as.factor(c(rep("AZ", each=41), rep("JW", each=41)))
```
GPA align 
```{r}
...
plotAllSpecimens(AJ_Morpho_Auto.gpa$coords)
```
Make geomorph dataframe and run manova
```{r}
AJ_Auto_GDF <- geomorph.data.frame(shape = AJ_Morpho_Auto.gpa$coords, individual = AJ_Auto_Groups$StudyID, observer=AJ_Auto_Groups$Observer)

AJ_Morpho_Auto.manova <- procD.lm(shape~observer+individual, data = AJ_Auto_GDF, print.progress=FALSE)
summary(AJ_Morpho_Auto.manova)
```
```{r}
plot(AJ_Morpho_Auto.manova, type = "diagnostics", outliers = TRUE)
```
##Compare manual vs automatic using manova on "Both" data, with individual and method as covariates. 
...
Groups dataframe that has sample information for combined manual landmarking data 
```{r}
Both_MLvsAuto_Groups <- as.data.frame(matrix(nrow=nrow(Both_MorphoData_MLvsAuto), ncol=3))
colnames(Both_MLvsAuto_Groups) <- c("ID", "StudyID", "Method")
Both_MLvsAuto_Groups$ID <- rownames(Both_MorphoData_MLvsAuto)
Both_MLvsAuto_Groups$StudyID <- as.factor(unique(landmarks$ID))
Both_MLvsAuto_Groups$Method <- as.factor(c(rep("ML", each=41), rep("Auto", each=41)))
```
...
plotAllSpecimens(Both_Morpho_MLvsAuto.gpa$coords)
```
Make geomorph dataframe and run manova
```{r}
Both_MLvsAuto_GDF <- geomorph.data.frame(shape = Both_Morpho_MLvsAuto.gpa$coords, individual = Both_MLvsAuto_Groups$StudyID,  method = Both_MLvsAuto_Groups$Method)

Both_Morpho_MLvsAuto.manova <- procD.lm(shape ~ method+individual, data = Both_MLvsAuto_GDF, print.progress=FALSE)
...
##Compare manual vs automatic using manova on AZ / JW trained data with individual, observer, and method as covariates
```{r}
AJ_MorphoData_MLvsAuto <- rbind(AZ_MorphoData_Manual_avg, JW_MorphoData_Manual_avg, AZ_MorphoData_Auto, JW_MorphoData_Auto)
```
Groups dataframe that has sample information for combined manual landmarking data 
```{r}
AJ_MLvsAuto_Groups <- as.data.frame(matrix(nrow=nrow(AJ_MorphoData_MLvsAuto), ncol=4))
colnames(AJ_MLvsAuto_Groups) <- c("ID", "StudyID", "Method", "Observer")
AJ_MLvsAuto_Groups$ID <- rownames(AJ_MorphoData_MLvsAuto)
AJ_MLvsAuto_Groups$StudyID <- as.factor(unique(landmarks$ID))
AJ_MLvsAuto_Groups$Method <- as.factor(c(rep("ML", each=82), rep("Auto", each=82)))
AJ_MLvsAuto_Groups$Observer <- as.factor(c(rep("AZ", each=41), rep("JW", each=41)))
```
GPA align manual and automatic landmark iterations
```{r}
...
plotAllSpecimens(AJ_Morpho_MLvsAuto.gpa$coords)
```
Make geomorph dataframe 
```{r}
AJ_MLvsAuto_GDF <- geomorph.data.frame(shape = AJ_Morpho_MLvsAuto.gpa$coords, individual = AJ_MLvsAuto_Groups$StudyID, method = AJ_MLvsAuto_Groups$Method, observer=AJ_MLvsAuto_Groups$Observer)

AJ_Morpho_MLvsAuto.manova <- procD.lm(shape~method+observer*individual, data = AJ_MLvsAuto_GDF, print.progress = F)
summary(AJ_Morpho_MLvsAuto.manova)
```
```{r}
plot(AJ_Morpho_MLvsAuto.manova, type = "diagnostic", outliers = TRUE)
```
...
colnames(CentroidSizes) <- c("ID", "A_Manual", "A_Auto", "J_Manual", "J_Auto", "B_Manual", "B_Auto")
...
CentroidSize_Melt$Observer <- as.factor(c(rep("AZ", each=82), rep("JW", each=82), rep("Both", each=82)))
