...
```{r}
ML_Inter$AJ_ml_rmsex <- apply(landmarks[4:ncol(landmarks)], 1, function(x) RMSE(x["A_ml_avgx"], x["J_ml_avgx"])) 
ML_Inter$AJ_ml_rmsey <- apply(landmarks[4:ncol(landmarks)], 1, function(x) RMSE(x["A_ml_avgy"], x["J_ml_avgy"]))
ML_Inter$AJ_ml_rmsez <- apply(landmarks[4:ncol(landmarks)], 1, function(x) RMSE(x["A_ml_avgz"], x["J_ml_avgz"]))

#Average
ML_Inter$AJ_ml_avg_rmse <- rowMeans(subset(ML_Inter, select = c(AJ_ml_rmsex, AJ_ml_rmsey, AJ_ml_rmsez)))
```
Average of RMSE x, y, z across all landmarks
```{r}
mean(ML_Inter$AJ_ml_rmsex)
mean(ML_Inter$AJ_ml_rmsey)
mean(ML_Inter$AJ_ml_rmsez)
```
Boxplot of RMSE x, y, z
```{r}
ggplot(data = melt(ML_Inter[, c("ID", "LM.Num", "LM.Name", "AJ_ml_rmsex", "AJ_ml_rmsey", "AJ_ml_rmsez")], id.vars = c("ID", "LM.Num", "LM.Name")), aes(x=LM.Name, y=value, fill=variable))+geom_boxplot()+theme_minimal()+theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust=0.1), legend.position = "right", legend.title = element_blank())+scale_fill_discrete(breaks=c("AJ_ml_rmsex", "AJ_ml_rmsey", "AJ_ml_rmsez"), labels=c("X", "Y", "Z"))+ylab("Root mean squared error (mm)")+xlab("Landmark")+ggtitle("Inter-observer error")
```
One high sdx value for chelion left
```{r}
ML_Inter[which(ML_Inter$AJ_ml_rmsex == max(ML_Inter$AJ_ml_rmsex)),]
```
 
##Average per landmark 
```{r}
ML_InterByLandmark <- ddply(ML_Inter,~LM.Name,summarise, AJ_ml_rmsex = mean(AJ_ml_rmsex), AJ_ml_rmsey = mean(AJ_ml_rmsey), AJ_ml_rmsez = mean(AJ_ml_rmsez), AJ_ml_avg_rmse = mean(AJ_ml_avg_rmse)) 
...
```{r}
#AZ
MLvsAuto$A_ml_loo_rmsex <- apply(landmarks[4:ncol(landmarks)], 1, function(x) RMSE(x['A_loo_x'], x['A_ml_avgx']))
MLvsAuto$A_ml_loo_rmsey <- apply(landmarks[4:ncol(landmarks)], 1, function(x) RMSE(x['A_loo_y'], x['A_ml_avgy']))
MLvsAuto$A_ml_loo_rmsez <- apply(landmarks[4:ncol(landmarks)], 1, function(x) RMSE(x['A_loo_z'], x['A_ml_avgz']))
MLvsAuto$A_ml_loo_avg_rmse <- rowMeans(subset(MLvsAuto, select = c(A_ml_loo_rmsex, A_ml_loo_rmsey, A_ml_loo_rmsez)))

#JW
MLvsAuto$J_ml_loo_rmsex <- apply(landmarks[4:ncol(landmarks)], 1, function(x) RMSE(x['J_loo_x'], x['J_ml_avgx']))
MLvsAuto$J_ml_loo_rmsey <- apply(landmarks[4:ncol(landmarks)], 1, function(x) RMSE(x['J_loo_y'], x['J_ml_avgy']))
MLvsAuto$J_ml_loo_rmsez <- apply(landmarks[4:ncol(landmarks)], 1, function(x) RMSE(x['J_loo_z'], x['J_ml_avgz']))
MLvsAuto$J_ml_loo_avg_rmse <- rowMeans(subset(MLvsAuto, select = c(J_ml_loo_rmsex, J_ml_loo_rmsey, J_ml_loo_rmsez)))
...
write.table(MLvsAuto_RMSEPerLMK, "Manuscript/Tables/MLvsAuto_RMSEPerLMK.txt", row.names = F, col.names = T, quote = F, sep = "\t")
...
##Comparison of Arslan ML - Arslan auto, Julie ML - Julie auto
...
```{r}
MLvsAuto$AJ_ml_avg_rmse <- ML_Inter$AJ_ml_avg_rmse
```
Plot
```{r}
ggplot(data = melt(MLvsAuto[, c("ID", "LM.Num", "LM.Name",  "AJ_ml_avg_rmse", "A_ml_loo_avg_rmse", "J_ml_loo_avg_rmse", "B_ml_loo_avg_rmse")], id.vars = c("ID", "LM.Num", "LM.Name")), aes(x=LM.Name, y=value, fill=variable))+geom_boxplot()+theme_minimal()+theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust=0.1), legend.position = "top", legend.title = element_blank())+scale_fill_discrete(breaks=c("AJ_ml_avg_rmse", "A_ml_loo_avg_rmse", "J_ml_loo_avg_rmse", "B_ml_loo_avg_rmse"), labels=c("RMSE between AZ and JW manual", "RMSE between AZ manual and auto", "RMSE between JW manual and auto", "RMSE between 'both' manual and auto"))+ylab("Root mean squared error (mm)")+xlab("Landmark")+ggtitle("Comparison of inter-observer errors\ncalculated using manual and automatic landmarks")
```
...
write.table(Inter_MLvsAuto_PerLMK, "Manuscript/Tables/Inter_MLvsAuto_PerLMK.txt", row.names = F, col.names = T, quote = F, sep = "\t")
