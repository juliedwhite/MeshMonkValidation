...
```{r}
MLvsAuto_RMSE_ByLandmark <- ddply(MLvsAuto,~LM.Name,summarise, Bml_Bloo_rmsex = mean(Bml_Bloo_rmsex), Bml_Bloo_rmsey = mean(Bml_Bloo_rmsey), Bml_Bloo_rmsez = mean(Bml_Bloo_rmsez), Bml_Bloo_avg_rmse = mean(Bml_Bloo_avg_rmse)) 

write.table(MLvsAuto_RMSE_ByLandmark, "Manuscript/Tables/MLvsAuto_Both_RMSE_PerLMK.txt", row.names = F, col.names = T, quote = F, sep = "\t")
```
...
MLvsAuto$Aml_Jml_rmsex <- ML_Inter$Aml_Jml_rmsex
MLvsAuto$Aml_Jml_rmsey <- ML_Inter$Aml_Jml_rmsey
MLvsAuto$Aml_Jml_rmsez <- ML_Inter$Aml_Jml_rmsez
```

Plot
```{r}
ggplot(data = melt(MLvsAuto[, c("ID", "LM.Num", "LM.Name",  "Aml_Jml_avg_rmse", "Aml_Aloo_avg_rmse","Aml_Jloo_avg_rmse","Jml_Jloo_avg_rmse", "Jml_Aloo_avg_rmse")], id.vars = c("ID", "LM.Num", "LM.Name")), aes(x=LM.Name, y=value, fill=variable))+geom_boxplot()+theme_minimal()+theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust=0.1), legend.position = "top", legend.title = element_blank())+scale_fill_discrete(breaks=c("Aml_Jml_avg_rmse", "Aml_Aloo_avg_rmse","Aml_Jloo_avg_rmse", "Jml_Jloo_avg_rmse", "Jml_Aloo_avg_rmse"), labels=c("Aml - Jml", "Aml - Aauto", "Aml - Jauto", "Jml - Jauto", "Jml - Aauto"))+ylab("Root mean squared error (mm)")+xlab("Landmark")
```
...
Jml - Aauto
```{r}
ggplot(data = melt(MLvsAuto[, c("ID", "LM.Num", "LM.Name",  "Aml_Jml_avg_rmse", "Aml_Jloo_avg_rmse", "Jml_Aloo_avg_rmse")], id.vars = c("ID", "LM.Num", "LM.Name")), aes(x=LM.Name, y=value, fill=variable))+geom_boxplot()+theme_minimal()+theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust=0.1), legend.position = "top", legend.title = element_blank())+scale_fill_discrete(breaks=c("Aml_Jml_avg_rmse", "Aml_Jloo_avg_rmse", "Jml_Aloo_avg_rmse"), labels=c("Aml - Jml", "Aml - Jauto", "Jml - Aauto"))+ylab("Root mean squared error (mm)")+xlab("Landmark")
```

Write table for manuscript
```{r}
tmp <- MLvsAuto[c("ID", "LM.Name", "Aml_Jml_rmsex", "Aml_Jml_rmsey", "Aml_Jml_rmsez", "Aml_Jml_avg_rmse", "Aml_Jloo_rmsex", "Aml_Jloo_rmsey", "Aml_Jloo_rmsez", "Aml_Jloo_avg_rmse", "Jml_Aloo_rmsex", "Jml_Aloo_rmsey", "Jml_Aloo_rmsez", "Jml_Aloo_avg_rmse")]

tmp_PerLMK <- ddply(.data = tmp, ~ LM.Name , summarize,
                    Aml_Jml_rmsex = mean(Aml_Jml_rmsex), 
                    Aml_Jml_rmsey = mean(Aml_Jml_rmsey), 
                    Aml_Jml_rmsez = mean(Aml_Jml_rmsez), 
                    Aml_Jml_avg_rmse = mean(Aml_Jml_avg_rmse), 
                    Aml_Jloo_rmsex = mean(Aml_Jloo_rmsex), 
                    Aml_Jloo_rmsey = mean(Aml_Jloo_rmsey), 
                    Aml_Jloo_rmsez = mean(Aml_Jloo_rmsez), 
                    Aml_Jloo_avg_rmse = mean(Aml_Jloo_avg_rmse),
                    Jml_Aloo_rmsex = mean(Jml_Aloo_rmsex), 
                    Jml_Aloo_rmsey = mean(Jml_Aloo_rmsey), 
                    Jml_Aloo_rmsez = mean(Jml_Aloo_rmsez), 
                    Jml_Aloo_avg_rmse = mean(Jml_Aloo_avg_rmse))

write.table(tmp_PerLMK, "Manuscript/Tables/AmlVsJml_AmlVsJauto_AautoVsJml_RMSE_PerLMK.txt", row.names = F, col.names = T, sep = "\t", quote = F)
```
...
```{r}
AmlVsJml_AmlVsJloo_RMSE.Ttest = melt(MLvsAuto[c("LM.Name", "Aml_Jml_avg_rmse", "Aml_Jloo_avg_rmse")], id.vars = "LM.Name") %>% group_by(LM.Name) %>% do(tidy(t.test(value~variable, data=., paired=TRUE)))

AmlVsJml_AmlVsJloo_RMSE.Ttest

write.table(AmlVsJml_AmlVsJloo_RMSE.Ttest, "Manuscript/Tables/AmlVsJml_AmlVsJauto_RMSE.Ttest.txt", row.names = F, col.names = T, sep = "\t", quote = F)
```
```{r}
AmlVsJml_AlooVsJml_RMSE.Ttest = melt(MLvsAuto[c("LM.Name", "Aml_Jml_avg_rmse", "Jml_Aloo_avg_rmse")], id.vars = "LM.Name") %>% group_by(LM.Name) %>% do(tidy(t.test(value~variable, data=., paired=TRUE)))

AmlVsJml_AlooVsJml_RMSE.Ttest

write.table(AmlVsJml_AlooVsJml_RMSE.Ttest, "Manuscript/Tables/AmlVsJml_AautoVsJml_RMSE.Ttest.txt", row.names = F, col.names = T, sep = "\t", quote = F)
```
...
```
Write this table for manuscript
```{r}
write.table(MLvsAuto_PerLMK, "Manuscript/Tables/MLvsAuto_All_RMSE_PerLMK.txt", row.names = F, col.names = T, sep = "\t")
```
...
```
#All the manovas
##Manova of manual landmarks only with individual, camera, observer, and iteration as covariates
...
```
Because one camera has only one individual represented, going to remove that individual from manova because no variation exists for that camera because of the sample size. 
```{r}
AJ_MorphoData_Manual <- AJ_MorphoData_Manual[!rownames(AJ_MorphoData_Manual) %like% "131213",]
ids <- unique(landmarks$ID)
id_subset <- ids[ids != "131213"]
```
...
####Regressing out individuals first: 
```{r}
#Make geomorph dataframe 
AJ_Manual_GDF <- geomorph.data.frame(shape = AJ_Morpho_Manual.gpa$coords, individual = as.character(AJ_Manual_Groups$StudyID), observer = AJ_Manual_Groups$Observer, iteration = AJ_Manual_Groups$Iteration, camera = AJ_Manual_Groups$Camera)

AJ_Morpho_Manual.manova <- procD.lm(f1 = shape ~ individual+observer/iteration+observer*individual, data = AJ_Manual_GDF, RRPP=TRUE, iter = 999, print.progress=FALSE)
summary(AJ_Morpho_Manual.manova)
```

```{r}
AJ_Morpho_Residuals <- as.data.frame(AJ_Morpho_Manual.manova$residuals)
```

```{r}
residual.pca <- prcomp(AJ_Morpho_Residuals, center = TRUE, scale = TRUE)
summary(residual.pca)
```

```{r}
AJ_Morpho_Residuals_PC1to40 <- as.data.frame(residual.pca$x[,1:40])
AJ_Morpho_Residuals_PC1to40$Camera <- AJ_Manual_Groups$Camera
```

```{r}
residual.manova <- manova(as.matrix(AJ_Morpho_Residuals_PC1to40[,1:40])~Camera, data = AJ_Morpho_Residuals_PC1to40)
summary(residual.manova)
```
####Regressing out camera first 
```{r}
AJ_Morpho_Manual.manova <- procD.lm(f1 = shape ~ camera, data = AJ_Manual_GDF, RRPP=TRUE, iter = 999, print.progress=FALSE)
summary(AJ_Morpho_Manual.manova)
```
...
```{r}
AJ_Morpho_Residuals_PC1to40 <- as.data.frame(residual.pca$x[,1:40])
AJ_Morpho_Residuals_PC1to40$Individual <- as.character(AJ_Manual_Groups$StudyID)
AJ_Morpho_Residuals_PC1to40$Observer <- AJ_Manual_Groups$Observer
AJ_Morpho_Residuals_PC1to40$Iteration <- AJ_Manual_Groups$Iteration
```

```{r}
residual.manova <- manova(as.matrix(AJ_Morpho_Residuals_PC1to40[,1:40])~Individual+Observer/Iteration+Observer*Individual, data = AJ_Morpho_Residuals_PC1to40)
summary(residual.manova)
```
```
##Manova of average manual landmarks with individual, camera, and observer as covariates
```{r}
AJ_MorphoData_AvgManual <- rbind(AZ_MorphoData_Manual_avg, JW_MorphoData_Manual_avg)
```
Remove individual imaged using unique camera
```{r}
AJ_MorphoData_AvgManual <- AJ_MorphoData_AvgManual[!rownames(AJ_MorphoData_AvgManual) %like% "131213",]
```
Make groups dataframe containing covariates
```{r}
AJ_AvgManual_Groups <- as.data.frame(matrix(nrow=nrow(AJ_MorphoData_AvgManual), ncol=4))
colnames(AJ_AvgManual_Groups) <- c("ID", "StudyID", "Camera", "Observer")
AJ_AvgManual_Groups$ID <- rownames(AJ_MorphoData_AvgManual)
AJ_AvgManual_Groups$StudyID <- rep(id_subset)
AJ_AvgManual_Groups$Camera <- c(rep("2005 3dMD 2-pod", each=14), rep("2014 3dMD 3-pod", each = 26))
AJ_AvgManual_Groups$Observer <- c(rep("AZ", each=40), rep("JW", each=40))
```
GPA align 
```{r}
AJ_Morpho_AvgManual <- arrayspecs(A=AJ_MorphoData_AvgManual, p=19, k=3)
AJ_Morpho_AvgManual.gpa <- gpagen(A=AJ_Morpho_AvgManual, print.progress = FALSE)
```
Plot outliers
```{r}
plotOutliers(AJ_Morpho_AvgManual.gpa$coords)
```
Make geomorph dataframe and run manova
```{r}
AJ_AvgManual_GDF <- geomorph.data.frame(shape = AJ_Morpho_AvgManual.gpa$coords, individual = AJ_AvgManual_Groups$StudyID, camera = AJ_AvgManual_Groups$Camera, observer=AJ_AvgManual_Groups$Observer)

AJ_Morpho_AvgManual.manova <- procD.lm(shape ~  individual*camera+observer, data = AJ_AvgManual_GDF, iter = 999, RRPP=TRUE, print.progress=FALSE)
summary(AJ_Morpho_AvgManual.manova)
```
```{r}
plot(AJ_Morpho_AvgManual.manova, type = "diagnostics", outliers = TRUE)
```

##Manova of auto landmarks with individual, camera, and observer as covariates.
anova of average auto landmarks with individual, camera, and observer as covariates
```{r}
AJ_MorphoData_Auto <- rbind(AZ_MorphoData_Auto, JW_MorphoData_Auto)
```
Remove individual imaged using unique camera
```{r}
AJ_MorphoData_Auto <- AJ_MorphoData_Auto[!rownames(AJ_MorphoData_Auto) %like% "131213",]
```
Make groups dataframe containing covariates
```{r}
AJ_Auto_Groups <- as.data.frame(matrix(nrow=nrow(AJ_MorphoData_Auto), ncol=4))
colnames(AJ_Auto_Groups) <- c("ID", "StudyID", "Camera", "Observer")
AJ_Auto_Groups$ID <- rownames(AJ_MorphoData_Auto)
AJ_Auto_Groups$StudyID <- rep(id_subset)
AJ_Auto_Groups$Camera <- c(rep("2005 3dMD 2-pod", each=14), rep("2014 3dMD 3-pod", each = 26))
AJ_Auto_Groups$Observer <- c(rep("AZ", each=40), rep("JW", each=40))
```
GPA align 
```{r}
AJ_Morpho_Auto <- arrayspecs(A=AJ_MorphoData_Auto, p=19, k=3)
AJ_Morpho_Auto.gpa <- gpagen(A=AJ_Morpho_Auto, print.progress = FALSE)
```
Plot outliers
```{r}
plotOutliers(AJ_Morpho_Auto.gpa$coords)
```
Make geomorph dataframe and run manova
```{r}
AJ_Auto_GDF <- geomorph.data.frame(shape = AJ_Morpho_Auto.gpa$coords, individual = AJ_Auto_Groups$StudyID, camera = AJ_Auto_Groups$Camera, observer=AJ_Auto_Groups$Observer)

AJ_Morpho_Auto.manova <- procD.lm(shape ~  individual*camera+observer, data = AJ_Auto_GDF, iter = 999, RRPP=TRUE, print.progress=FALSE)
summary(AJ_Morpho_Auto.manova)
```
```{r}
plot(AJ_Morpho_Auto.manova, type = "diagnostics", outliers = TRUE)
```
##Manova of manual "both" landmarks with individual and camera as covariates
Because one camera has only one individual represented, going to remove that individual from manova because no variation exists for that camera because of the sample size. 
```{r}
Both_MorphoData_ManualSubset <- Both_MorphoData_Manual[!rownames(Both_MorphoData_Manual) %like% "131213",]
```
Make geomorph style dataframe
```{r}
Both_Morpho_ManualSubset <- arrayspecs(A=Both_MorphoData_ManualSubset, p = 19, k = 3)
```
Align individuals
```{r}
Both_Morpho_ManualSubset.gpa <- gpagen(A=Both_Morpho_ManualSubset, print.progress = FALSE)
```
Groups dataframe that has sample information for  manual landmarking data 
```{r}
Both_Manual_Groups <- as.data.frame(matrix(nrow=nrow(Both_MorphoData_ManualSubset), ncol=3))
colnames(Both_Manual_Groups) <- c("ID", "StudyID", "Camera")
Both_Manual_Groups$ID <- rownames(Both_MorphoData_ManualSubset)
Both_Manual_Groups$StudyID <- rep(id_subset)
Both_Manual_Groups$Camera <- c(rep("2005 3dMD 2-pod", each=14), rep("2014 3dMD 3-pod", each = 26))
```
Plot outliers
```{r}
plotOutliers(Both_Morpho_ManualSubset.gpa$coords)
```
Make geomorph dataframe and run manova
```{r}
Both_ManualSubset_GDF <- geomorph.data.frame(shape = Both_Morpho_ManualSubset.gpa$coords, individual = Both_Manual_Groups$StudyID, camera = Both_Manual_Groups$Camera)

Both_Morpho_ManualSubset.manova <- procD.lm(shape ~ individual*camera, data = Both_ManualSubset_GDF, iter = 999, RRPP=TRUE, print.progress=FALSE)
summary(Both_Morpho_ManualSubset.manova)
```
```{r}
plot(Both_Morpho_ManualSubset.manova, type = "diagnostics", outliers = TRUE)
```

##Manova on auto "both" landmarks with individual and camera as covariates
Because one camera has only one individual represented, going to remove that individual from manova because no variation exists for that camera because of the sample size. 
```{r}
Both_MorphoData_AutoSubset <- Both_MorphoData_Auto[!rownames(Both_MorphoData_Auto) %like% "131213",]
```
Make geomorph style dataframe
```{r}
Both_Morpho_AutoSubset <- arrayspecs(A=Both_MorphoData_AutoSubset, p = 19, k = 3)
```
Align individuals
```{r}
Both_Morpho_AutoSubset.gpa <- gpagen(A=Both_Morpho_AutoSubset, print.progress = FALSE)
```
Groups dataframe that has sample information for  Auto landmarking data 
```{r}
Both_Auto_Groups <- as.data.frame(matrix(nrow=nrow(Both_MorphoData_AutoSubset), ncol=3))
colnames(Both_Auto_Groups) <- c("ID", "StudyID", "Camera")
Both_Auto_Groups$ID <- rownames(Both_MorphoData_AutoSubset)
Both_Auto_Groups$StudyID <- rep(id_subset)
Both_Auto_Groups$Camera <- c(rep("2005 3dMD 2-pod", each=14), rep("2014 3dMD 3-pod", each = 26))
```
Plot outliers
```{r}
plotOutliers(Both_Morpho_AutoSubset.gpa$coords)
```
Make geomorph dataframe and run manova
```{r}
Both_AutoSubset_GDF <- geomorph.data.frame(shape = Both_Morpho_AutoSubset.gpa$coords, individual = Both_Auto_Groups$StudyID, camera = Both_Auto_Groups$Camera)

Both_Morpho_AutoSubset.manova <- procD.lm(shape ~ individual*camera, data = Both_AutoSubset_GDF, iter = 999, RRPP=TRUE, print.progress=FALSE)
summary(Both_Morpho_AutoSubset.manova)
```
```{r}
plot(Both_Morpho_AutoSubset.manova, type = "diagnostics", outliers = TRUE)
```
...
AJ_Morpho_MLvsAuto.manova <- procD.lm(shape ~  observer*camera*individual + method, data = AJ_MLvsAuto_GDF, iter = 999, RRPP=TRUE, print.progress=FALSE)
...
```{r}
PLS <- NULL
PLS$AZ_Manual_PC1 <- AZ_PLS$A1.matrix[,1]
PLS$AZ_Auto_PC1 <- AZ_PLS$A2.matrix[,1]
PLS$JW_Manual_PC1 <- JW_PLS$A1.matrix[,1]
PLS$JW_Auto_PC1 <- JW_PLS$A2.matrix[,1]
PLS$Both_Manual_PC1 <- Both_PLS$A1.matrix[,1]
PLS$Both_Auto_PC1 <- Both_PLS$A2.matrix[,1]
PLS <- as.data.frame(PLS)

multiplot(
  ggplot(PLS, aes(AZ_Manual_PC1, AZ_Auto_PC1))+geom_point()+geom_smooth(method = "lm")+theme_bw()+ylab("Automatic")+xlab("Manual"),
  ggplot(PLS, aes(JW_Manual_PC1, JW_Auto_PC1))+geom_point()+geom_smooth(method = "lm")+theme_bw()+ylab("Automatic")+xlab("Manual"),
  ggplot(PLS, aes(Both_Manual_PC1, Both_Auto_PC1))+geom_point()+geom_smooth(method="lm")+theme_bw()+ylab("Automatic")+xlab("Manual"), 
  cols = 3)
```
...
ba_centroid_Jdf <- as.data.frame(matrix(NA, nrow = 41))
...
```{r}
cs_histogram <- ggplot(data=melt(CentroidSizes, id.vars = c("ID", "Camera")), aes(x=variable, y=value))+geom_boxplot(outlier.shape = NA)+geom_jitter(width = 0.2, size = 0.8, alpha = 0.5)+labs(x="Measurement method", y="Centroid size (mm)")+theme_bw()+scale_x_discrete(breaks = c("A_Manual", "A_Auto", "J_Manual", "J_Auto", "B_Manual", "B_Auto"), labels = c("AZ manual", "AZ auto", "JW manual", "JW auto", "Both manual", "Both auto"))

cs_AZ <- ggplot(CentroidSizes, aes(A_Manual, A_Auto))+geom_point()+theme_bw()+labs(title = "AZ", x="", y="Automatic CS (mm)")
cs_JW <- ggplot(CentroidSizes, aes(J_Manual, J_Auto))+geom_point()+theme_bw()+labs(title = "JW",x="Manual CS (mm)", y="")
cs_Both <- ggplot(CentroidSizes, aes(B_Manual, B_Auto))+geom_point()+theme_bw()+labs(title = "B", x="", y="")

ggarrange(cs_AZ, cs_JW, cs_Both, ba_centroid_Aplot, ba_centroid_Jplot, ba_centroid_Bplot, ncol = 3, nrow=2, labels = c("(A)", "", "", "(B)", "", ""))
```
...
```{r}
CentroidSize_Melt <- melt(CentroidSizes[CentroidSizes$ID != "131213",], id.vars = c("ID", "Camera"))
CentroidSize_Melt$ID <- as.character(CentroidSize_Melt$ID)
CentroidSize_Melt$Observer <- c(rep("AZ", each=80), rep("JW", each=80), rep("Both", each=80))
CentroidSize_Melt$Method <- c(rep("ML", each=40), rep("Auto", each=40), rep("Manual", each=40), rep("Auto", each=40), rep("Manual", each=40), rep("Auto", each=40))

CentroidSize_AOV <- aov(value ~ ID + Observer + Method + Camera, data = CentroidSize_Melt)
